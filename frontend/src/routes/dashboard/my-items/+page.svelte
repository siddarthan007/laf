<script lang="ts">
	...
	import { getMyItems, resolveItem, deleteItem } from '$lib/api/items';
	...
	import { Package, MapPin, Calendar, CheckCircle, Trash2, Archive, AlertCircle, Search, X, Eye, ZoomIn, Filter } from '@lucide/svelte';
...
<div class="space-y-8">
	<div class="space-y-2">
		<h1 class="text-3xl md:text-4xl font-bold tracking-tight">My Items</h1>
		<p class="text-muted-foreground text-lg">View and manage your reported items</p>
	</div>
			...
			{:else if allActiveItems.length === 0}
				<Card>
					<CardContent class="py-16 text-center">
						...
						<p class="text-muted-foreground font-medium text-lg mb-1">No active items</p>
						<p class="text-sm text-muted-foreground">Start by reporting a lost or found item</p>
					</CardContent>
				</Card>
			{:else if allActiveItems.length === 0}
				<Card>
					<CardContent class="py-16 text-center">
						...
						<p class="text-muted-foreground font-medium text-lg mb-1">
							{searchQuery || locationFilter ? 'No items found matching your filters' : 'No active items'}
						</p>
						<p class="text-sm text-muted-foreground">
							{searchQuery || locationFilter ? 'Try adjusting your search or filters' : 'Start by reporting a lost or found item'}
						</p>
					</CardContent>
				</Card>
			{:else}
						...
						>
							<CardContent class="p-0">
								...
								{#if item.image_url}
									<button
										type="button"
											class="relative w-full h-48 bg-muted overflow-hidden rounded-xl flex items-center justify-center cursor-pointer hover:opacity-90 transition-opacity group opacity-75"
										onclick={(e) => openImageModal(getImageUrl(item.image_url!), item.description, e)}
									>
										<img
											src={getImageUrl(item.image_url)}
								...
								{:else}
										<div class="w-full h-48 bg-muted rounded-xl flex items-center justify-center opacity-75">
										<Package class="h-12 w-12 text-muted-foreground" />
									</div>
								{/if}
								</div>
		...
		<Dialog.Content class="max-w-3xl max-h-[90vh] overflow-y-auto">
			{#if selectedItem}
				<Dialog.Header>
					...
					<Dialog.Description>
						Complete information about your reported item
					</Dialog.Description>
				</Dialog.Header>
						...
						<div>
							...
							<div class="flex flex-wrap gap-2">
								...
								{#if selectedItem.has_match_found}
									<Badge variant="default" class="bg-green-600">Match Found</Badge>
								{/if}
								{#if selectedItem.is_admin_report}
