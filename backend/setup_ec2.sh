...
sudo dnf update -y
# Install Python 3.11, git, gcc, postgresql-devel
sudo dnf install -y python3.11 python3.11-devel git gcc postgresql-devel acl
...
# Remove old venv if it exists to avoid conflicts
if [ -d "$VENV_DIR" ]; then
    rm -rf "$VENV_DIR"
fi
